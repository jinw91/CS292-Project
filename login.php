...
if (isset($_SESSION['idnum']))
{
	...
	if ($_POST['login'] == "Log In")
	{
		...
		else
		{
			...
			$query = sprintf("SELECT b.b_id, company_name, sector, description, city, state, picture, founded FROM business_structure bs, businesses b WHERE idnum=%d AND bs.b_id=b.b_id", $_SESSION['idnum']);
			$result = mysql_query($query);
			if ($result && mysql_num_rows($result) > 0)
			{
